/* Código de programa para nodo sensor corregido (fecha y hora se actualizan y guarda Trm en SD)
 *  
 *  TFG:
...
void SendSensor(){
  ...
  //double Trm = pow((pow((Tg+273),4) + 2.5*pow(10,8)*pow(Va,0.6)*(Tg-Ta)),0.25) - 273;
  Trm = pow((pow((Tg+273),4) + 2.5*pow(10,8)*pow(Va,0.6)*(Tg-Ta)),0.25) - 273;
...
void Storage() {
  ...
  nowdate = now.timestamp(DateTime::TIMESTAMP_DATE);
  nowtime = now.timestamp(DateTime::TIMESTAMP_TIME);
  ...
  String dataString = nowdate+"   "+nowtime+"\t"+String(Ha)+"\t"+String(Ta)+"\t"+String(ppm)+"\t  "+String(value2)+"\t      "
  +String(lux)+"\t"+String(Hg)+"\t"+String(Tg)+"\t"+String(To)+"\t"+String(Trm);
  ...
  if (!SD.begin(chipSelect,SPI_HALF_SPEED)){       // Chequeo
    ...
    if (logFile) {                                 // Si está disponible, escribe en él
      ...
      Serial.println(F("Fecha\t     Hora\tHR(%)\tTs(ºC)\tCO2(ppm)  CRn(Bq/m3)  Lum(lx)\tHg(%)\tTg(ºC)\tTo(ºC)\tTrm(ºC)"));
      Serial.println(dataString);                  // Envia al monitor serie el mismo string
      Serial.println();
      Serial.println(F("*****************************************************************"));
    }
    else {                                         // Si no está disponible, fallo
      Serial.println(F("Fallo en el fichero"));  
    }
  }
}
...
void setup() {
  ...
  Serial.println(date.timestamp(DateTime::TIMESTAMP_FULL));
  Serial.println(" "); 
  ...
  if(i == 21){
    ...
    if(j == 21){
      Serial.print("No conectado a Blynk");
      delay(500);
    }
    else {
  ...
  // Puesta en marcha, configuración e información del sensor TSL2591
  ...
  //timer.setInterval(60000L, Storage);

  // Información del sensor GDK101
  Serial.println(F("==============================================="));
  ...
  if (!SD.begin(chipSelect,SPI_HALF_SPEED)) {      // Chequeo
    ...
    logFile = SD.open(fileName, FILE_WRITE);       // Abre el fichero
    if (logFile) {                                 // Si está disponible, escribe en él
      logFile.println(F("Fecha\tHora\tHR(%)\tTa(ºC)\tCO2(ppm)\tRn(Bq/m^3)\tLuminosidad(lx)\tHg(%)\tTg(ºC)\tTo(ºC)\tTrm(ºC)"));
      logFile.close();                             // Cierra el fichero
