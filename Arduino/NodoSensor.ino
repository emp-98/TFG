...
#define BLYNK_PRINT Serial                         // Definición para poder mostrar los 
                                                   // datos en la aplicación Blynk
// Se incluyen las librerías necesarias:
// Para el microcontrolador ESP8266
...
void SendSensor(){
  ...
  Ta = dht.readTemperature()+0.6134;               // Se obtiene la temperatura seca del ambiente
  ...
  // Asocia los datos con las variables virtuales de Blynk
  ...
  Tg = dht_g.readTemperature()-1.3761;             // Se obtiene la temperatura de globo
...
void setup() {
  ...
  Serial.println(F("Nodo sensor para control de calidad de aire con estimación de temperatura operativa basado en hardware abierto")); 
  ...
  // Mientras no esté conectado, se reintenta 20 veces y si no, sólo se guardan los datos en la SD sin conectarse a Blynk 
  uint8_t i = 0;                                   // Se declara aquí para que se ponga a 0 cada vez que se haga el setup
  uint8_t j = 0;
  ...
  //timer.setInterval(60000L, DatosTarjetaSD);
...
void loop() {
  ...
  DateTime ahora = rtc.now();                      // Obtención de la fecha y hora actual
  String ahoraStr = ahora.toString(buf);           // Pasa a tipo "String"
  // Datos que se quieren guardar y mostrar por el monitor serie
  String dataString = ahoraStr+"\t"+String(Ha)+"\t"+String(Ta)+"\t"+String(ppm)+"\t  "+String(value2)+"\t      "+String(lux)+"\t"+String(Hg)+"\t"+String(Tg)+"\t"+String(To);
  ...
  delay(900000);                                    // Lo hace cada 15min
 }
